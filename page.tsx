...
/* ================= CAPA DE DATOS — CONEXIÓN REAL A SUPABASE ================= */

import { supabase } from './supabaseClient'; // Importamos tu nuevo archivo de conexión
...
async function consultarEnvio(dni) {
  const { data, error } = await supabase
    .from("predicciones")
    .select("enviado, fecha_envio")
    .eq("dni", String(dni).trim())
    .maybeSingle();
  if (error) throw error;
  return { enviado: !!data?.enviado, fecha: data?.fecha_envio };
}

async function savePrediction(payload) {
  if (prodeCerrado()) throw new Error("PRODE_CERRADO");
  const { error } = await supabase
    .from("predicciones")
    .insert({ ...payload, enviado: true });
  if (error) throw error;
  return { ok: true };
}
/* ------------------------------- UI: ÁTOMOS ------------------------------- */
